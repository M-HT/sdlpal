...
static int find_dst_port(NativeMidiSong *song, const char *midi_address)
{
    ...
    {
        snd_seq_addr_t addr;
        if (snd_seq_parse_address(song->sequencer, &addr, midi_address) < 0) return -1;
...
static int open_alsa_port(NativeMidiSong *song)
{
    ...
    if (find_dst_port(song, getenv("MIDIUNIT")) < 0)
    {
        snd_seq_close(song->sequencer);
        song->sequencer = NULL;
        return -6;
        ...
        do
        {
            ...
            switch (event.type)
            {
                case SND_SEQ_EVENT_NOTEON:
                    ...
                    event.data.note.velocity = (events[current_event].data2 * song->volume * 517) >> 16; // data2 * song->volume / 127
